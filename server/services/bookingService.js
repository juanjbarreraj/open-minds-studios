...
import {
  todayInAppTz, addDays, weekdayName, isValidDateString, isValidTimeString, timeToMinutes,
} from '../lib/time.js';
...
export function createBooking({ tutor, student, payload }) {
  ...
  const insert = db.transaction(() => {
    const existing = db
      ...
      .get(tutor.id, session_date, preferred_start_time);
...
export function transitionBooking({ bookingId, actor, nextStatus }) {
  ...
  const stamps = {};
  if (nextStatus === 'cancelled') stamps.cancelled_at = new Date().toISOString();
  if (nextStatus === 'declined') stamps.declined_at = new Date().toISOString();
